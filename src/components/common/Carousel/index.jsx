    ...
    return (
        <div className={styles['carousel']}>
            <div className={styles['operation']}>
                <button className={styles['last']} onClick={throttle(500, () => {
                    handleClick(1)
                })}>上一个
                </button>
                <button className={styles['next']} onClick={throttle(500, () => {
                    handleClick(2)
                })}>下一个
                </button>
            </div>
            {
                [test1, test2, test3, test4].map((n, index) => {
                    return (
                        <div
                            className={[(index + 1) === active ? styles.active : (index + 1) === last ? styles.last : styles.next, styles.img].join(' ')}
                            key={index}>
